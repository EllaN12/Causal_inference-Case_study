...
def read_csv_files(file_paths):
    ...
    for file_path in file_paths:
        ...
        file_name = os.path.basename(file_path).replace('.csv', '_df')
        dataframes.append((df, file_name))
        
    return dataframes

# Define the file path and get the list of CSV files
file_path = '/Users/ellandalla/Desktop/Causal_inference-Case_study/venv/00_Raw_Data/*.csv'
files_path = glob.glob(file_path)

# Get the DataFrames along with their filenames
...
print(dataframes[0][0].head())


chefmozparking_df = dataframes[0][0]
chefmozaccepts_df = dataframes[1][0]
userpayment_df = dataframes[2][0]
restaurant_geoplaces2_df = dataframes[3][0]
user_rating_final = dataframes[4][0]
usercuisine_df = dataframes[5][0]
chefmozcuisine_df = dataframes[6][0]
chefmozhours4_df = dataframes[7][0]
userprofile_df = dataframes[8][0]
...
profile_patrons = ProfileReport(patrons_df, title='Pandas Profiling Report', explorative=True)

patrons_df.profile_report().to_file('/Users/ellandalla/Desktop/Causal_inference-Case_study/venv/01_Data_analysis/reports/patrons_report.html')
#restaurant_df.profile_report().to_file('/Users/ellandalla/Desktop/Causal_inference-Case_study/venv/01_Data_analysis/reports/restaurant_report.html')
# %%
...
categoty_data_df['smoker']= categoty_data_df['smoker'].replace('?', np.nan)
categoty_data_df['days']= categoty_data_df['days'].replace('Mon;Tue;Wed;Thu;Fri;', 'weekdays')
...
corr_df = df_binarized.correlate(target='mean')

restaurant_rating_correlated = .correlate(target='rating')

corr_df = df_binarized.reset_index().select_dtypes(include=['float64', 'int64']).correlate(target='rating')
