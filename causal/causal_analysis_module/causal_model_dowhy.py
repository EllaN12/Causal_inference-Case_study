...
model_lst = create_causal_models(df = df,
...
estimands_lst = generate_causal_estimands(
    models= model_lst)
...
def estimate_causal_effects(models, estimands, method_names=None):
    ...
    for treatment, model in models.items():
        if estimands[treatment] is None:
            print(f"Skipping estimation for {treatment} as no valid estimand was identified")
            continue
        ...
        for method_name in method_names:
            try:
                ...
                if method_name.startswith("iv.") and not has_valid_instruments(model, treatment):
                    print(f"Skipping {method_name} for {treatment} as no valid instruments were identified")
                    continue
                
                print(f"Estimating effect of {treatment} using {method_name}...")
...
estimate_lst = estimate_causal_effects(
    models = model_lst,
    estimands = estimands_lst)

#%%
# Try Analysis One at a time:
@pf.register_dataframe_method
def analyze_treatment_effect(df, treatment):
    """
    ...
    model = create_causal_models(
        df=df,
        treatment_lst=[treatment]  # for each treatment
    )[treatment]  # Extract the single model
    ...
    print("\nEstimating causal effect of {treatment} on rating:")
    print("------------------------------------------------")
...
@pf.register_dataframe_method
def visualize_treatment_effects(results, treatment):
    """
    ...
    plt.xlabel('Causal Effect of {treatment} on Overall Rating')
    plt.title('Estimated Causal Effect of {treatment} by Method')
...
@pf.register_dataframe_method
def run_treatment_analysis(df, treatment):
    """
    ...
    results = analyze_treatment_effect(df)
    
    if results:
        # Visualize the results
        visualize_treatment_effects(results)
        
        # Provide a textual interpretation
        print("\nInterpretation of {treatment} Causal Effect:")
        print("-------------------------------------------")
        
        # Calculate average effect across methods
        effects = [est.value for est in results['estimates'].values()]
        avg_effect = sum(effects) / len(effects)
        
        print(f"The average causal effect of {treatment} on overall rating is {avg_effect:.4f}.")
...
@pf.register_dataframe_method
def get_potential_instruments(treatment):
    """
...
@pf.register_dataframe_method
def run_iv_estimation_for_treatment(df, treatment, outcome='rating'):
    """
    ...
    model = create_causal_models(
        df=df,
        treatment_lst=[treatment],
    )[treatment]  # Extract the single model
    
    # Step 2: Identify the estimand
    try:
        estimand = model.identify_effect()
        print(f"✓ Successfully identified estimand for {treatment}")
    except Exception as e:
        print(f"✗ Error identifying estimand for {treatment}: {str(e)}")
        return {
            'treatment': treatment,
    ...
    instruments = get_potential_instruments(treatment)
    
    if not instruments:
        print(f"✗ No potential instruments identified for {treatment}")
        return {
            'treatment': treatment,
            'estimand': estimand,
    ...
    print(f"Potential instruments for {treatment}: {', '.join(instruments)}")
    ...
    try:
        ...
        print(f"✓ IV estimation successful for {treatment}")
        ...
        print(f"✗ Error in IV estimation for {treatment}: {str(e)}")
        return {
            'treatment': treatment,
            'estimand': estimand,
