...
_HERE         = Path(__file__).resolve().parent        # 04_Mediation_HTE/
_PIPELINE_DIR = REPO_ROOT / "01_Data_Analysis"
_RESULTS_DIR  = _HERE / "results"
_RESULTS_DIR.mkdir(parents=True, exist_ok=True)

for _p in (REPO_ROOT, _PIPELINE_DIR):
...
class MediationAnalysis:
    ...
    def analyze_treatment(self, treatment_col, treatment_name=None):
        ...
        if abs(c_total) > 0.001:
            ...
            if abs(total_indirect) > 0.001:
                ...
                food_relative = 0
                service_relative = 0
                print(f"\n⚠ Total indirect effect is negligible - mediator split not meaningful")
        else:
    ...
    def plot_mediation_results(self, save_path=None):
        """
        Visualize mediation analysis results
        """
        if save_path is None:
            save_path = str(_RESULTS_DIR / "mediation_results.png")
...
def run_mediation_pipeline(df, treatments=None):
    ...
    if len(analyzer.results) > 0:
        ...
        if summary_df is not None:
            summary_df.to_csv(str(_RESULTS_DIR / "mediation_summary.csv"), index=False)
            print(f"\n✓ Summary saved to '{_RESULTS_DIR / 'mediation_summary.csv'}'")
...
if __name__ == "__main__":
    ...
    # Output file lives in the phase results folder.
    output_path = _RESULTS_DIR / "causal_model_results.txt"

    # Tee: capture everything that goes to stdout/stderr while also printing it.
    ...
    with open(output_path, "w", encoding="utf-8") as fh:
        ...
        with contextlib.redirect_stdout(tee), contextlib.redirect_stderr(tee):
            df = get_initial_data()
            analyzer, summary_df = run_mediation_pipeline(df)
